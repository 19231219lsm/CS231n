...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      dists[i][j] = np.sqrt(torch.sum((x_train[i] -x_test[j])**2))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  #   # 找到k个点对应的标签
  #   closest_y = y_train[closest_k]
  #   # 统计出现次数并得到预测结果
  #   y_pred[i] = np.bincount(closest_y).argmax()
  for j in range(num_test):
    test_data = dists[:, j]
    dist_closest, idx = torch.topk(test_data, k=k, largest=False, sorted=True)
    y_closest = y_train[idx]
    bincount = torch.bincount(y_closest)
    y_pred[j] = torch.argmax(bincount)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
  ...
  for k in k_choices:
    ...
    for fold in range(num_folds):
      classifier = KnnClassifier(
        x_train=torch.cat(x_train_folds[:fold] + x_train_folds[fold + 1:], dim=0),
        y_train=torch.cat(y_train_folds[:fold] + y_train_folds[fold + 1:], dim=0)
      )
      x_test = x_train_folds[fold:fold + 1][0]
      y_test = y_train_folds[fold:fold + 1][0]
      res.append(classifier.check_accuracy(x_test, y_test, k))
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in sorted(k_to_accuracies):
    accuracy = max(k_to_accuracies[k])
